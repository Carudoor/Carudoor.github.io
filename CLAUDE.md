...
5. **3D 미리보기의 자동 회전은 잡고 있는 동안만 멈추고, 손을 떼면 즉시 재개된다**
   (`viewer.html`과 동일한 방식). `autoRotate`는 계속 true이고 `isDragging`으로만
   게이트한다.
   - 히스토리: 처음에는 "손 뗀 후 2.5초 뒤 재개"였는데 **타이머 방식은 삭제 요청**을
     받아 "한 번 조작하면 영구 정지"로 바꿨고, 이후 사용자가 다시 **뷰어와 동일하게
     (즉시 재개)** 해달라고 요청해 현재 방식이 되었다.
   - **타이머로 재개하는 방식은 쓰지 말 것.** 멈춤/재개는 오직 `isDragging` 기준.
   - 터치도 동일하게 동작한다(`touchstart`에서 멈추고 `touchend`에서 재개).
     `viewer.html`도 같은 방식으로 맞춰두었다.
...
- **공유 자산은 `?v=` 버전을 달고 부른다**(`slides-core.js?v=2` 등).
  매니페스트 형식과 뷰어 코드를 함께 바꿔 배포하면, 재방문자 브라우저에 남은
  **캐시된 옛 JS가 새 매니페스트를 읽어 화면이 깨진다**(실제로 겪었다).
  형식이나 동작을 바꿀 때는 이 숫자를 반드시 올릴 것.
